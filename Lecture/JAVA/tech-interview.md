...
어차피 스프링 컨테이너는 기본적으로 싱글톤을 보장하고요  
직접 개발자가 구현하는것은 OCP 와 DIP 을 위배하는 행위가 되는겁니다.
...
또, 스프링 빈에서도 리플랙션이 사용됩니다.  
`@Configuration`이 붙어있는 클래스에 한해, `@CGLIB` 라는 바이트코드를 조작하는 기술이 빈의 싱글톤을 유지해줍니다.
...
## HTTP1과 HTTP2 차이점을 간단히
HTTP1에서는 하나의 요청가 응답을 처리하기 때문에 동시전송 문제와 다수의 리소스를 처리하는데 속도와 성능 이슈를 가지고 있습니다.  
HTTP2에서는 Multiplexed Streams 를 통해 여러개의 메시지를 동시에 주고 받아 성능을 향상 시켰습니다. 

## Mysql 만 쓰셨는데 NoSQL 은 고려를 안해봤나요?

우선 springboot+JPA 조합이 실무에서 RDB 구조로 많이 쓰이고,  
sql 을 통해 데이터를 가져오는 정형화된 방법이 그때 저의 이해도에서는 최선이였습니다.  
(어떻게 보면 NoSQL을 아예 고려 하지 않은 것이죠.)

지금 와서 생각하라고 해도 NoSQL 의 장점을 살릴 수 있을 정도의 도메인은 아닌 것 같아요.  
NoSQL 을 사용하면 RDBMS를 사용했을 때의 데이터와 트래픽의 기하급수적인 비용의 한계에 부딪힐 수 있다고 하지만, 저희가 scale-out 까지 고려하기에는 너무 작고요.
...
RDBMS - 데이터 변경이 자주 이루어지는 시스템에 적합
NoSQL - 데이터 변경이 적고 막대한 데이터를 저장해야 해서 Scale-out 해야 하는 시스템에 적합
...
단점
* 테이블간 관계에 있어서 JOIN문이 많은 복잡한 쿼리가 만들어 질 수 있다.
* 성능 향상을 위해서는 서버의 성능을 향상 시켜야 하는 Scale-up 만 지원한다.
* 스키마로 인해 데이터가 유연하지 못하다.

### NoSQL
장점
* 유연한 데이터 구조, 언제든 저장된 데이터를 조정하고 새로운 필드를 추가할 수 있음
* 데이터 분산이 용이 성능 향상을 위한 Scale-up 뿐만 아니라 Scale-out도 가능

단점
* 데이터의 중복 발생이 가능하다.
  * 발생할 경우 수정을 모든 컬렉션에서 수행해야 한다.
...
- 클러스터드 인덱스
  - 테이블당 1개씩
  - PK 설정 시 그 칼럼은 자동으로 클러스터드 인덱스가 된다.
  - 검색 속도는 빠르지만 입력, 수정, 삭제 시간은 느리다.
  - 페이지 번호를 알고 그 페이지 내용을 찾는 것
...
### B-Tree
- RDBMS에서 대부분 사용하는 자료구조 임.
- 주로 Full scan을 진행, 검색속도 저하.

## 정렬

### 버블정렬
* 자신의 원소 옆 원소와 정렬 기준에 맞게 크기를 비교하여 정렬하는 방법
* 최선, 평균, 최악이 O(N^2) 같음
* 알고리즘 구현이 쉽지만, 성능이 안좋음

### 선택정렬
* 자신의 원소와 그 위치 pt를 미리 저장하고 pt 이후의 인덱스 값 중에서 최솟값을 찾아 pt 와 교환한다.