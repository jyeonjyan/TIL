...
## JPA 등장배경 그리고 사용하면 좋은 점

<img src="../../img/JPA-history.png" width="600px">

EJB라는 엔티티빈이 있었는데 성능도 안좋고 로직이 복잡해져서 어떤 SI 개발자 분에 의해 등장한게 하이버네이트다.  
근데 그 하이버네이트 개발자가 능력을 인정받아 EJB를 만든 java 진영에서 JPA를 같이 개발했다고 한다.

### 그래서 JPA를 사용해야 하는 이유가 뭔데?

딱 두가지로 정리해보려고 한다.
* 객체와 DB 사이의 패러다임 불일치를 해결하기 위해서
* 생산성인 측면이 압도적이기 때문에 (SQL은 JPA가 알아서 만들어 준다, 변경시 설정을 일일히 다 바꿔줄 필요 없다.. 등)
* 성능에서의 이점을 보일 수 있기 때문에 (지연 로딩과 즉시 로딩.. 등)
...
### 객체와 DB 사이의 패러다임 불일치 해결

이걸 알아보려면 JPA가 Application 과 JDBC 사이에서 어떤 역할을 하는지 알아야 한다.

<img src="../../img/JPA-JDBC.png" width="600px">

이렇게 JPA는 애플리케이션과 JDBC API 사이에 위치해 app의 요청을 받아 JDBC API를 사용하여 알맞는 쿼리를 만든다.  
이러한 특징이 결과적으로 개발자가 sql을 작성할 필요도 없고, 패러다임의 불일치를 해결해준다.

### 영속성 컨텍스트의 이점
> 영속성이란 것은 "엔티티를 영구 저장하는 환경"이라는 뜻이다.  
> 자세히 들어가면 플러시라는 개념도 나오는데 이는 영속성 컨텍스트의 변경 내용을 DB에 반영하는 것이다.

결과적으로 영속성 컨텍스트, 엔티티를 영구 저장하는 환경
